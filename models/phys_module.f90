...
module phys_module
  ...
  integer :: physics_pc_dump_blocks !< Workstream C, SFM2 only: 0 = off (default). /= 0 writes pair_psi, pair_w, S_uu, B_11, B_33, B_44 as PETSc binary files plus gmgdump_grid.txt (node indices, element vertices and size factors) to the run directory at every SFM2 rebuild, for the offline geometric-multigrid probe docs/physics_pc/tools/gmg_probe.py. Run with physics_pc_pair_scale = 0 so pair_psi is dumped unscaled.
