...
module phys_module
  ...
  logical :: physics_pc_sf !< Select the clean production preconditioner: the SFM2 block-LDU sweep with S_uu = B_22 + W (the composed force operator), two mixed 2x2 pairs, and per block the C1 geometric multigrid or the exact LU it is gated against. Requires physics_pc_force_operator = 1. Forces physics_pc_harm_split = 1 and the audited GMG smoother / axis-ring / boundary-drop settings, loudly. Everything that was a measurement variable rather than a design choice is absent by construction, including every verify/probe/report/dump routine and the per-rebuild diagnostics they used to run.
  character(len=16) :: physics_pc_sf_pair_psi !< Production path: solver for pair_psi = [[B_11,B_13],[B_31,B_33]]. "gmg" (default) = FGMRES + one C1 GMG V-cycle on the pair itself: psi and j kept split through the cycle and smoothed collectively by zebra radial lines (Chacon JCP 526 (2025) S4.1); no Schur approximation, no B_33 solve, and eta_num > 0 allowed. "lu" = PREONLY + LU (MUMPS), the exact reference.
