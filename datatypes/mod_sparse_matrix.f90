module mod_sparse_matrix
  ...
  type type_SP_MATRIX
    ...
    real(kind=8), dimension(:), pointer          :: row_scaling => Null()       !< global row scaling, vector size of ng
    ...
    integer                                      :: ncpu = 1
    logical                                      :: scaled = .false. !TODO: might be redundant with equilibrated flag, keeping for backwards compatibility
    logical                                      :: equilibrated = .false. !< Wether matrix has been equilibrated (row and column scaled)
...
! move one matrix structure into another
  subroutine move_to(self, mat_a, with_data)
    ...
    if (with_data) then
      ...
      mat_a%row_scaling    => self%row_scaling
    ...
    mat_a%equilibrated    = self%equilibrated
...
! copy one matrix structure into another
  subroutine copy_to(self, mat_a)
    ...
    if (associated(self%row_scaling)) then
      allocate(mat_a%row_scaling(mat_a%ng))
      mat_a%row_scaling(1:mat_a%ng) = self%row_scaling(1:self%ng)
    endif
  ...
  subroutine reset(self)
    ...
    if (associated(self%row_scaling)) then
      deallocate(self%row_scaling); self%row_scaling => Null()
    endif
    if (associated(self%index_min)) then
